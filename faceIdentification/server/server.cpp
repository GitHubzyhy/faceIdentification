    ...
    , ui(new Ui::Widget)
{
    ...
    thread->start();
    //绑定查询槽函数
    connect(this, &Server::query, &fobj, &QFaceObject::face_query);
    //关联QFaceObject对象里面的send_faceid信号
    connect(&fobj, &QFaceObject::send_faceid, this, &Server::recv_faceid);
}
...
void Server::initSocket()
{
            ...
            this, [this]
    {
        ...
        connect(m_socket, &QTcpSocket::readyRead, this, &Server::read_data);
    });
}
...
void Server::read_data()
{
    ...
    queryFace(data);//查询是否为人脸
}

// 查询是否为人脸
void Server::queryFace(const QByteArray &data)
{
    //-----------------识别人脸------------------//
...
{
    QSqlQuery query;
    model.setTable("employee"); //给sql模型绑定表格

    //清空考勤表
    query.exec("DELETE FROM  attendance");
    query.exec("DELETE FROM  sqlite_sequence WHERE name =\'attendance\'");
}

void Server::recv_faceid(int64_t faceid)
{
