...
void Client::initSocket()
{
    ...
    //接受json数据槽函数连接
    connect(&m_socket, &QTcpSocket::readyRead, this, &Client::recv_data);
    //启动定时器,每5s连接一次直到成功
    m_timer.start(5000);
    m_socket.connectToHost(QHostAddress::LocalHost, 9999);//构造的时候先连接一次
}

//向服务器发送数据
void Client::sendData(const Rect &rect)
{
...
void Client::timerEvent(QTimerEvent *event)
{
    ...
    // 将 Mat 数据转换为 QImage
    QImage image(srcImage.data, srcImage.cols, srcImage.rows,
                 srcImage.step1(), QImage::Format_RGB888);
